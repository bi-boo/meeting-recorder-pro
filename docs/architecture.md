...
- **输出格式**：当前公开版承诺 M4A/AAC；MP3 入口保留运行时可用性保护，不随默认构建分发第三方编码器
- **热键管理**：Carbon (全局快捷键注册)
...
- **输出格式**：默认并公开承诺 M4A；MP3 仅在运行环境提供可用编码器时出现在设置中
...
## H2 MP3Encoder
MP3 输出统一入口保留为可选扩展点。录音阶段仍先写入 M4A；只有运行时确认 MP3 编码器可用时，设置界面才展示 MP3 选项并在停止后异步转码。

- **转换流程**：读取 M4A → 分块解码 PCM → 可用编码器写入 MP3
- **内存控制**：每次处理 8192 帧，避免长录音一次性解码到内存
- **可用性处理**：MP3 编码不可用时隐藏 MP3 选项，并将已保存的 MP3 偏好回落为 M4A
