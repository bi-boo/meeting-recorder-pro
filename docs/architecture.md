...
- **热键管理**：Carbon (全局快捷键注册)
- **存储**：UserDefaults (配置与状态持久化) + Keychain (自动录音任务授权摘要)
...
### H3 签名规约
- **环境安全**：只从项目根目录 `.env` 加载明确的发布变量白名单；严格发布还要求干净工作区和可校验的产物来源清单
...
- **自动录音授权**：应用内新增、编辑或启停自动录音计划时，对完整任务状态生成 SHA-256 摘要并保存到本机 Keychain。调度器在触发与真正启动录音前都会比对摘要；仅修改 `UserDefaults` 不能获得自动录音授权。
- **升级迁移**：旧版已启用的自动录音计划因没有 Keychain 摘要，首次升级启动时保留计划但自动切为关闭；用户需在应用内重新启用或保存后，才会恢复无人值守录音。纯提醒计划不受影响。
