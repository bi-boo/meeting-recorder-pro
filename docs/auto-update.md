...
- 安装方式：用户点击菜单栏更新操作项后，由 Sparkle 下载、验签、替换应用并重启安装
- 录音保护：录音中禁止检查更新，也不会启动安装流程

## 菜单栏行为

- 操作项默认文案：`检查更新...`
- 后台发现更新：操作项显示 `下载并安装 x.y.z...`
- 状态项常驻显示：`当前版本 x.y.z`
- 更新流程状态：按阶段显示 `正在检查更新...`、`正在下载更新...`、`正在准备安装...`、`正在安装并重启...`
- 重启完成确认：安装前记录目标版本，重启后比对当前版本；确认成功后显示 `已更新到 x.y.z`
- 录音中：更新操作项禁用

后台检查发现更新时不弹出更新窗口，只更新菜单项。用户主动点击“下载并安装 x.y.z...”后，不展示更新窗口和版本说明，直接进入 Sparkle 的下载、验签、重启安装流程；如果系统要求管理员授权，仍可能出现系统授权提示。
