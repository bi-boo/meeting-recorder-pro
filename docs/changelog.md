# [待发布 1.0.7 安全加固]
- **录音文件保护**：MP3 转换完成后重新验证可播放性、音轨和时长，只有最终文件有效时才删除源 M4A。
- **定时录音授权**：自动录音计划增加本机 Keychain 授权摘要，外部修改 `UserDefaults` 不能直接获得无人值守录音权限；旧版自动录音计划升级后需在应用内重新启用。
- **发布链加固**：发布变量改为项目根目录白名单，Python 发布依赖使用全新临时环境和 SHA-256 锁定，并用来源清单绑定 Git 提交、App 与 DMG。
- **隐私与测试**：收敛录音日志中的设备和文件信息，处理公开截图中的个人路径，并补充安全退出与测试环境边界。
- **验证状态**：37 项单元测试与 8 项发布安全测试通过；本机 DMG 构建通过。系统声音、混合录音和部分人工交互场景仍需在具备对应系统权限的正式发布环境补测。
